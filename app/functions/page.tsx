      ...
      <div className='flex my-20 flex-col h-full items-center justify-center text-center'>
        ...
        <div className=' flex flex-col justify-center items-center text-center gap-8 mx-8 my-12'>
          ...
          <p className='w-full flex justify-center items-center h-full text-base'>
            A function is a line of code that will be used multiple times. Essentially in scenarios where you need to do the same thing twice, you can create a function, and instead of copy and pasting that same code twice, you only need to call upon that function twice, with the code only being created once. 
          </p>
          <h2>
            Parts of a function
          </h2>
          <p className='w-full flex justify-center items-center h-full text-base'>
            Functions basically just look like this: def functionName( parameter1, paremeter2, ...): ‘’’ Description of what the function does ‘’’ #body of the function return expression. Def is required for you to create the function, it is defining the function. functionName is the name of the function, which like all forms of documentation has to be clear on the purpose. parameter# is a parameter that the function accepts. The definition of a parameter is a variable that is defined in the function definition. The parameters can only be used inside the function, and are treated just like variables. It can also be blank if the function doesn’t need to manipulate any information. ‘’’ Description ‘’’ is used to be able to describe the purpose of the function. The body is what your function will do, and it is usually short, as it should usually be used to perform tasks repeatedly. The return will return back to the part of your code where you called on the function, or return a value. It doesn’t always need to return a variable. 
          </p>
          <h2>
            Parameter Details
          </h2>
          <p className='w-full flex justify-center items-center h-full text-base'>
            Functions ALWAYS need to be defined before actually using them within the code, as such they always need to be at the top of the code. Any variables that are inside a function only exist within tht function, and functions should only use parameters to receive information and shouldn’t be the one using an input to be able to get the information. Furthermore, variables are passed into functions by placing them within the brackets of the function being called. For example, if I have a function called functionName and it has 3 parameters, I would pass in 3 variables like so: functionName(variable1, variable2, variable3). 
          </p>
          <h2>
            Code Example
          </h2>
          <p className='w-full flex justify-center items-center h-full text-base'>
            Then in the def functionName(name, age, date) the variable1 will become the name variable that is used within the function, variable2 will become age, variable3 becomes date. In the code found above, you can see the defining of the function, along with the variables that will be used multiple times throughout the code. Then there is the body of the code where it does all it needs to do, to keep on creating a grid for our wordle game. Then, at the very end of the function you will see it return the pastList, and resultList, which are two things that we need at the area of the code where the function is being called. As a note, functions can contain subprograms, and is very simillar to any code that is outside of the function, but instead if placed inside of it.
          </p>
        </div>
      </div>
      <div>
